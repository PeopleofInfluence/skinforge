...
export default function SkinForgePage() {
  ...
  // Expose undo/redo functions from PixelEditor
  ...
  const setTool = useCallback((tool: Tool) => {
    setEditorState((s) => ({ ...s, tool }));
  }, []);

  const setColor = useCallback((color: string) => {
    setEditorState((s) => ({ ...s, color }));
  }, []);

  const setLayer = useCallback((activeLayer: LayerName) => {
    setEditorState((s) => ({ ...s, activeLayer }));
  }, []);
  ...
  const zoomIn = useCallback(() => {
    setEditorState((s) => ({ ...s, zoom: Math.min(s.zoom + 2, 20) }));
  }, []);

  const zoomOut = useCallback(() => {
    setEditorState((s) => ({ ...s, zoom: Math.max(s.zoom - 2, 2) }));
  }, []);

  const toggleGrid = useCallback(() => {
    setEditorState((s) => ({ ...s, showGrid: !s.showGrid }));
  }, []);

  const setBodyType = useCallback((bodyType: BodyType) => {
    setEditorState((s) => ({ ...s, bodyType }));
  }, []);
  ...
  const handlePixelsChange = useCallback((imageData: ImageData) => {
    ...
    // Undo/redo availability is managed inside usePixelEditor,
    // so we track it here loosely by the existence of history
    setCanUndo(true);
  }, []);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-2 bg-forge-panel border-b border-forge-border shrink-0">
        <div className="flex items-center gap-2">
          <Logo />
          <span className="text-sm font-semibold text-forge-text tracking-wide">
            SkinForge
          </span>
          <span className="text-xs text-forge-text-muted hidden sm:block">
